...
export class LoginComponent {
  ...
  constructor(public appSettings:AppSettings, public fb: FormBuilder, public router:Router, private httpserv: HttpClient, private mainservice:MainService){
    ...
    this.UserSubmit.User_Password = this.newPass;  //<<<< set new password to post
     this.httpserv.post<User>("https://localhost:44329/api/User/Login",this.UserSubmit).subscribe(   // <<< Post to LOGIN API endpoint
       (data:any) => {
         ...
         //localStorage.setItem("currentUserID", "5")
 
         localStorage.setItem("loggedIn", "Yes");       // <<< set global logged in attribut
